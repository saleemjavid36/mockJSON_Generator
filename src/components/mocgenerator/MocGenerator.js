...
const MocGenerator = () => {
  ...
  const generateMockData = (count) => {
    if (count > 100000) {
      alert("Too much data to download. Please reduce the number of records.");
    } else {
      ...
      link.download = "User-Mock-Data.json";
      link.click();
    }
  ...
  const handleInputChange = (e) => {
    ...
    setTotalRecordsCount(Number(value));
  }
    ...
    <div className="col-12 d-flex justify-content-center   vh-100">
      <div className="my-5 d-flex flex-column align-items-center">
        <div>
          ...
          <input value={totalRecordsCount ?? 0} type="number" onChange={handleInputChange} />
        </div>
        <h1>Generate Mock JSON Data</h1>
        <button onClick={() => generateMockData(totalRecordsCount)}>Generate and Download JSON</button>
      </div>
